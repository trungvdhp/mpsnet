...
double calculateContourAngle(const vector<Point2i>& contour, int contourSize, int index, int step, double maxAngle) {
    ...
    const int backward = index - step;
    const int forward = (index + step) % contourSize;
...
vector<Point2i> samplingLine(const Point2i& p1, const Point2i& p2, int maxX, int maxY) {
    ...
    const int b = cross(p2, p1);
    int t;

    if (p.x == 0 && p.y == 0) return points;

    if (abs(p.x) > abs(p.y)) {
        const int sign_x = sign(p.x);
...
Intersection intersection2Vectors(const Point2i& t1, const Point2i& h1, const Point2i& t2, const Point2i& h2
    ...
    const auto d11 = pointLineDistance(t1, f2);
    const auto d12 = pointLineDistance(h1, f2);
    const auto d21 = pointLineDistance(t2, f1);
    const auto d22 = pointLineDistance(h2, f1);
    ...
    f1.c = -f1.c;
    f2.c = -f2.c;
    const auto d = cross4(f1.a, f1.b, f2.a, f2.b);
    const auto dx = cross4(f1.c, f1.b, f2.c, f2.b);
    const auto dy = cross4(f1.a, f1.c, f2.a, f2.c);
    inter.d1 = f1.d;
    inter.d2 = f2.d;
...
vector<Point2i> bisector(const Point2i& t1, const Point2i& h1, const Point2i& t2, const Point2i& h2, bool& direction) {
    ...
    res.at(0) = midpoint(t1, t2);
    res.at(1) = midpoint(h1, h2);
    res.at(2) = res.at(1);
    ...
    if (inter.p.x == INT_MAX) {
        ...
        const auto id1 = (inter.d12 < 0 || inter.d22 < 0) ? -1 : midByRate(saturate_cast<int>(ps1.size()), inter.d12, inter.d22);
        res.at(1) = (id1 < 0 || id1 >= ps1.size()) ? midpoint(h1, h2) : ps1.at(id1);
        auto ps2 = samplingLine(t1, t2);
        const auto id2 = (inter.d11 < 0 || inter.d21 < 0) ? -1 : midByRate(saturate_cast<int>(ps2.size()), inter.d11, inter.d21);
        res.at(2) = (id2 < 0 || id2 >= ps2.size()) ? midpoint(t1, t2) : ps2.at(id2);
    }
...
vector<ConvexHull> correctHull(const Mat& img, const vector<int>& cvHull, const vector<Point2i>& contour, vector<double>& angles, size_t minHullEdgeSize, size_t maxHullEdgeSize,  size_t minHullContourPoints, double maxHullAngle, double maxContourAngle) {
    ...
    const int contourSize = saturate_cast<int>(contour.size());
    const int cvHullSize = saturate_cast<int>(cvHull.size());
    vector<ConvexHull> newHull;
    auto prev = cvHull.at(cvHull.size() - 1);
    angles = vector<double>(saturate_cast<unsigned long>(contourSize));
    size_t diff = 0;
    int max_d = 0;
    int max_id = prev;
    int max_first = prev;
    ...
    for (int i = cvHullSize - 1; i >= 0; --i) {
        ...
        while (j < end) {
            ...
            if (new_seg) {
                ...
                const int curr_dist = max_d;
                last_zero = j;

                while (j < end) {
                    angle = calculateContourAngle(contour, contourSize, j);
                    j < 0 ? angles.at(j + contourSize) = angle : angles.at(j) = angle;
                        ...
                        if (dist == 0.0 || (dist - nde > minHullEdgeSize && j - nid > minHullEdgeSize && max_d - nde > minHullEdgeSize && max_d - dist > minHullEdgeSize)) {
                            if (diff > minHullEdgeSize) {
                                if (max_id > j - minHullEdgeSize)
                                    max_id = saturate_cast<int>(j - minHullEdgeSize);

                                const Point2i pprev = valueAt(contour, contourSize, prev);
                                const Point2i pnid = valueAt(contour, contourSize, nid);
                                mid = midValue(max_first, max_id);
                                const Point2i pmax = valueAt(contour, contourSize, mid);
                                angle = angle3pt(pprev, pmax, pnid);
                                d1 = distance2Points(pnid, pprev);

                                if (angle<maxHullAngle && d1>minHullEdgeSize && d1<maxHullEdgeSize && max_d>minHullEdgeSize && max_d < maxHullEdgeSize) {

                                    if (prev > j) {
                                        prev -= contourSize;
                                    }

                                    if (dist == 0 || d1 < 5 * minHullEdgeSize) {
                                        bw = make_pair(10, 0);
                                    }
                                    else {
                                    ...
                                    if (bw.second < 0.5 * bw.first) {
                                        ...
                                        if (temp_size > 0) {
                                            auto flag = true;
                                            const Point2i pbegin = valueAt(contour, contourSize, temp.at(temp_size - 1).begin);
                                            const Point2i pend = valueAt(contour, contourSize, temp.at(temp_size - 1).end);
                                            const auto a1 = angle3pt(pbegin, pprev, pend);

                                            if (a1 >= maxContourAngle) flag = false;
                                            else {
                                                const auto a2 = angle3pt(pbegin, pnid, pend);

                                                if (a2 >= maxContourAngle) flag = false;
                                                else {
                                                    const auto a3 = angle3pt(pprev, pbegin, pnid);

                                                    if (a3 >= maxContourAngle) flag = false;
                                                    else {
                                                        const auto a4 = angle3pt(pprev, pend, pnid);
                                            ...
                                            if (flag) {
                                                ...
                                                if (j - prev > temp.at(temp_size - 1).end - temp.at(temp_size - 1).begin) {
                                                    temp.back() = hull;
                                                }
                                            }
                                        }
        ...
        if (diff > minHullEdgeSize) {
            ...
            if (max_id > last - minHullEdgeSize) {
                max_id = saturate_cast<int>(last - minHullEdgeSize);
            }

            const Point2i pprev = valueAt(contour, contourSize, prev);
            const Point2i plast = valueAt(contour, contourSize, last);
            mid = midValue(max_first, max_id);
            const Point2i pmax = valueAt(contour, contourSize, mid);
        ...
        if (temp_size == 0) {
            ...
            if (angle<maxHullAngle && d1>minHullEdgeSize && d1<maxHullEdgeSize && global_max_dist>minHullEdgeSize && global_max_dist < maxHullEdgeSize) {
            ...
            if (max_d > temp.at(temp_size - 1).dist) {
                temp.back().dist = max_d;
                temp.back().farthest = midValue(max_id, max_first);
            }

            v1 = valueAt(contour, contourSize, temp.back().begin);
            v2 = valueAt(contour, contourSize, temp.back().end);
            const Point2i pmax = valueAt(contour, contourSize, temp.back().farthest);
            angle = angle3pt(v1, pmax, v2);
            d1 = distance2Points(v1, v2);

            if (angle<maxHullAngle && d1>minHullEdgeSize && d1<maxHullEdgeSize && temp.back().dist>minHullEdgeSize && temp.back().dist < maxHullEdgeSize) {

                bw = countBlackWhite(img, v1, v2);

                if (bw.second < 0.5 * bw.first) {
                    temp.back().area = contourArea(
                        subVector(contour, contourSize,
                            temp.back().begin, temp.back().end));
                }
                else {
                    temp.pop_back();
                    temp_size--;
        ...
        if (temp_size > 1) {
            ...
            while (temp.at(temp_size - 1).area < 0.1 * areas) {
                temp_size--;
            }
            if (temp_size > 4) {
                temp_size = 4;
        ...
        if (temp_size > 0) {
            ...
            for (size_t k = 0; k < temp_size; ++k) {
                if (temp.at(k).dist > max_d) {
                    max_d = temp.at(k).dist;
                }
            }
            if (temp_size == 1 || max_d > minHullContourPoints) {
    ...
    sort(newHull.begin(), newHull.end(),
        [](const ConvexHull& a, const ConvexHull& b) {
            return saturate_cast<int>(a.dist > b.dist);
        });
...
bool isValidValleyEdge(const Point2i& edgeBegin, const Point2i& edgeEnd, const Point2i& hullBegin, const Point2i& hullEnd, const double distanceThreshold) {
    const auto intersection = intersection2Vectors(
        edgeBegin,
        edgeEnd,
        hullBegin,
        hullEnd
...
ValleyEdge findValleyEdges(const vector<Point2i>& contour, const vector<double>& angles, const ConvexHull& hull, double angleThreshold) {
    ...
    int edge_length = (hull.farthest - hull.begin + 1) / 3;
    ...
    // Search from the farthest point
    edge_length = (hull.end - hull.farthest + 1) / 3;
...
int lineContourIntersectionId(const vector<Point2i>& contour, const LineCoefficient& coef, const Point2i& ref, int contourSize, int beginId, int endId, int step, bool flag) {

    int id = midValue(beginId, endId);
    int minDistance = INT_MAX, maxDistance = 0, distance = 0;
    Point2i p;
...
pair<Point2i, Point2i> findReferenceLine(const vector<Point2i>& contour, const LineCoefficient& coef, const Point2i& ref, const Valley& v1, const Valley& v2) {
    ...
    const int contourSize = saturate_cast<int>(contour.size());
    const int expand = contourSize / 5;
    ...
    const int k = distance2Points(v1.center, v2.center) / 2;
    int curr_dist = 0;
...
BoundingBox getROICoordinates(const Mat& binary_image, const vector<Point2i>& contour, const Valley& v1, const Valley& v2, const Point2i& ref, int minROISize, int maxROISize) {
    ...
    const auto mid1 = midpoint(v1.center, v2.center);
    const auto n = distance2Points(v1.center, v2.center);
    BoundingBox bbox;
    ...
    auto p = sub(ref, mid1);
    p.x = saturate_cast<int>(mid1.x + sign(p.x) * k1);
    p.y = saturate_cast<int>(mid1.y + sign(p.y) * m * k1);
    ...
    const auto refLine = findReferenceLine(contour, coef, p, v1, v2);
    const auto refDistance = distance2Points(refLine.first, refLine.second) * (1.0 - marginLeft * 2);

    if (refDistance<minROISize || refDistance>maxROISize) {
        errorString = "Out_of_distance";
        return bbox;
    }
    const auto mid2 = midpoint(refLine.first, refLine.second);
    ...
    mid_top_edge.x = saturate_cast<int>(mid1.x + sign(p.x) * k1);
    mid_top_edge.y = saturate_cast<int>(mid1.y + sign(p.y) * m * k1);

    mid_bottom_edge.x = saturate_cast<int>(mid_top_edge.x + sign(p.x) * k2);
    mid_bottom_edge.y = saturate_cast<int>(mid_top_edge.y + sign(p.y) * m * k2);
    ...
    top_left.x = saturate_cast<int>(mid_top_edge.x + sign(top_left.x) * k1);
    top_left.y = saturate_cast<int>(mid_top_edge.y + sign(top_left.y) * m * k1);

    auto top_right = sub(mid1, v2.center);
    top_right.x = saturate_cast<int>(mid_top_edge.x + sign(top_right.x) * k1);
    top_right.y = saturate_cast<int>(mid_top_edge.y + sign(top_right.y) * m * k1);
    ...
    bottom_left.x = saturate_cast<int>(mid_bottom_edge.x + sign(bottom_left.x) * k1);
    bottom_left.y = saturate_cast<int>(mid_bottom_edge.y + sign(bottom_left.y) * m * k1);

    auto bottom_right = sub(mid1, v2.center);
    bottom_right.x = saturate_cast<int>(mid_bottom_edge.x + sign(bottom_right.x) * k1);
    bottom_right.y = saturate_cast<int>(mid_bottom_edge.y + sign(bottom_right.y) * m * k1);

    // Validate roi coordinates
    const int x_min = 0;
    const int y_min = 0;
    const int x_max = binary_image.cols - 1;
    const int y_max = binary_image.rows - 1;
...
vector< vector<int> > getValleyTriplets(const vector<Valley>& valleys) {
    ...
    const auto valleysSize = valleys.size();
            ...
            if (j != i) {
                ...
                for (int k = 0; k < valleysSize; ++k) {
                    ...
                    if (k != i && k != j) {
                        ...
                        angle = saturate_cast<int>(angle3pt(
                            valleys.at(j).center,
                            valleys.at(i).center,
                            valleys.at(k).center
                        ));
...
vector< pair<int, int> > selectTwoOptimalKeyVectors(const vector<Valley>& valleys, int angle, int angleThreshold) {
    ...
    const auto d1 = distance2Points(valleys.at(1).center, valleys.at(0).center);
    const auto d2 = distance2Points(valleys.at(1).center, valleys.at(2).center);
    const auto delta = max(d1, d2) / 2;
...
BoundingBox selectROICandidate(const Mat& binaryImage, const vector<Point2i>& contour, const vector<Valley>& valleys, int minROISize, int maxROISize, int angleThreshold) {
    ...
    const auto angle_t = angleThreshold + 30;
    ...
    if (valleysSize == 2) {
        ...
        valley1 = valleys.at(0);
        valley2 = valleys.at(1);
        ...
        if (angle1 > 0) {
            valley1 = valleys.at(1);
            valley2 = valleys.at(0);
        }
        ...
        for (auto item : triplets) {
            ...
            vector<Valley> triplet{ valleys.at(item.at(1)),valleys.at(item.at(2)),valleys.at(item.at(3)) };
            auto candidates = selectTwoOptimalKeyVectors(triplet, item.at(0), angleThreshold);

            if (candidates.empty()) continue;

            for (auto c : candidates) {

                valley1 = triplet.at(c.first);
                valley2 = triplet.at(c.second);
                angle1 = angle2VectWithDirection(
                    sub(valley1.center, valley1.ref),
                    sub(valley1.center, valley2.center));
                angle2 = angle2VectWithDirection(
                ...
                if (angle1 > 0) {
                    valley1 = triplet.at(c.second);
                    valley2 = triplet.at(c.first);
                }
                begin1 = valueAt(contour, contourSize, valley1.hull.begin);
                end1 = valueAt(contour, contourSize, valley2.hull.end);
                d1 = distance2Points(begin1, end1);
...
void segmentHand(const Mat& rgbImage, Mat& binaryImage, vector<Point2i>& contour) {
    ...
    const auto thresholding_method = THRESH_OTSU;
    ...
    const auto bin_size = binaryMaskSize * binaryMaskSize;
    ...
    binary.setTo(0);
    drawContours(binary, contours, saturate_cast<int>(largest_contour_index), Scalar(1.0), FILLED);
    contours.clear();
    ...
    //morphologyEx(binary, binary, MORPH_ERODE, cv::getStructuringElement(MORPH_ELLIPSE, cv::Size(3, 3)));
    ...
    contour = contours.at(largest_contour_index);

    binaryImage = Mat(binary.rows, binary.cols, CV_8UC1, Scalar(0));
    drawContours(binaryImage, contours, saturate_cast<int>(largest_contour_index), Scalar(255.0), FILLED);
}
...
void extractPalmROI(const Mat& rgbImage, Mat& roiImage, Mat& binaryImage, Mat& croppedImage, int roiSize) {
    ...
    Mat resizedImage;
    const bool portrait = rgbImage.cols < rgbImage.rows;
    cv::Rect rect;

    if (portrait) {
        const int biasY = (rgbImage.rows - rgbImage.cols) / 2;
        rect = cv::Rect(0, biasY, rgbImage.cols, rgbImage.cols);
    }
    else {
        const int biasX = (rgbImage.cols - rgbImage.rows) / 2;
    ...
    roiImage = binaryImage;

    // Step 3. Find potential valleys between the adjacent fingers
    const int contourSize = saturate_cast<int>(contour.size());
    vector<int> cvHull;
    vector<double> angles;
    const double maxContourAngle = 170.0;
    const double maxHullAngle = 100;
    const size_t minHullEdgeSize = 2;
    const size_t maxHullEdgeSize = 80;
    const size_t minHullContourPoints = 16;
    const int minROISize = saturate_cast<int>(binaryMaskSize * rmin);
    const int maxROISize = saturate_cast<int>(binaryMaskSize * rmax);
    int begin_id, end_id, inter_id;
    LineCoefficient coef;
    
    convexHull(contour, cvHull, false, false);
    auto hulls = correctHull(binaryImage, cvHull, contour, angles, minHullEdgeSize, maxHullEdgeSize, minHullContourPoints, maxHullAngle, maxContourAngle);
    // Step 4. Find key vector candidates
    vector<Valley> valleys;

    for (auto hull : hulls) {
        const Point2i pb = valueAt(contour, contourSize, hull.begin);
        const Point2i pe = valueAt(contour, contourSize, hull.end);
        ValleyEdge ve;
        ...
        coef = getLineCoefficient(bs.at(0), bs.at(1));
        inter_id = lineContourIntersectionId(contour, coef, bs.at(1), contourSize, begin_id, end_id, sign(end_id - begin_id), direction);

        v.cid = inter_id;
        v.ref = (distance2Points(head1, bs.at(1)) > minHullEdgeSize) ? bs.at(1) : bs.at(2);
    ...
    const int angleThreshold = 120;
    bbox = selectROICandidate(binaryImage, contour, valleys, minROISize, maxROISize, angleThreshold);
    //cout << "Error: " << errorString << endl;
    //cout << "Edge size: " << bbox.edgeSize << endl;

    if (bbox.edgeSize == INT_MAX) return;

    // Draw palm ROI on binaryImage
    line(binaryImage, bbox.topLeft, bbox.topRight, Scalar(0), 1);
    line(binaryImage, bbox.topRight, bbox.bottomRight, Scalar(0), 1);
    line(binaryImage, bbox.bottomRight, bbox.bottomLeft, Scalar(0), 1);
    line(binaryImage, bbox.bottomLeft, bbox.topLeft, Scalar(0), 1);
    circle(binaryImage, bbox.topLeft, 2, Scalar(0), -1);
    circle(binaryImage, bbox.topRight, 2, Scalar(0), -1);
    circle(binaryImage, bbox.bottomLeft, 2, Scalar(0), -1);
    circle(binaryImage, bbox.bottomRight, 2, Scalar(0), -1);

    // Scale up to croppedImage size
    const double scaleFactor = croppedImage.rows * 1.0 / binaryMaskSize;
    bbox.scale(scaleFactor);

    // Step 7. Extract palm ROI
    const auto dsize = cv::Size(roiSize, roiSize);
    const auto input4 = bbox.inputQuad();
    const auto output4 = BoundingBox::outputQuad(roiSize);
    auto M = getPerspectiveTransform(input4, output4);
    roiImage = Mat(roiSize, roiSize, CV_8UC1);
    warpPerspective(croppedImage, roiImage, M, dsize);

    // Step 8. Extract saturation image
    //roiImage = extractSaturationFromRGBImage(roiImage);
    
    // Draw palm ROI on croppedImage
    line(croppedImage, bbox.topLeft, bbox.topRight, Scalar(0), 2);
    line(croppedImage, bbox.topRight, bbox.bottomRight, Scalar(0), 2);
    line(croppedImage, bbox.bottomRight, bbox.bottomLeft, Scalar(0), 2);
    line(croppedImage, bbox.bottomLeft, bbox.topLeft, Scalar(0), 2);
    circle(croppedImage, bbox.topLeft, 4, Scalar(0), -1);
    circle(croppedImage, bbox.topRight, 4, Scalar(0), -1);
    circle(croppedImage, bbox.bottomLeft, 4, Scalar(0), -1);
    circle(croppedImage, bbox.bottomRight, 4, Scalar(0), -1);
}

int main() {
    string test_dir = "D:/Test/";
    string filename = "ntust_ip_s1";
    string ext = ".png";
    const int roiSize = 128;

    string input_path = test_dir + filename + ext;
    Mat bgrImage = imread(input_path, -1);
    Mat rgbImage, roiImage, binaryImage, croppedImage;
    cvtColor(bgrImage, rgbImage, COLOR_BGR2RGB);
    extractPalmROI(rgbImage, roiImage, binaryImage, croppedImage, roiSize);

    string output_roi_path = test_dir + filename + "_roi" + ext;
    cvtColor(roiImage, roiImage, COLOR_RGB2BGR);
    imwrite(output_roi_path, roiImage);

    string binary_image_path = test_dir + filename + "_binary" + ext;
    imwrite(binary_image_path, binaryImage);

    string output_cropped_image = test_dir + filename + "_crop" + ext;
    cvtColor(croppedImage, croppedImage, COLOR_RGB2BGR);
    imwrite(output_cropped_image, croppedImage);
